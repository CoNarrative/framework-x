...
const connectFn = (name, config, renderFn) => {
  ...
  class SyntheticComponentBasedOnRenderFunction extends Component<any, any> {
...
export const component = (name: string, mapStateOrConfigBag: any, renderFn?: (...any) => any) => {
  if (renderFn == null) {
      ...
      render() {
        return (
          <Context.Consumer>
            {({ dispatch }) =>
              // @ts-ignore
              renderFn(Object.assign({ dispatch }, this.props))}
          </Context.Consumer>
        )
      }
    }
