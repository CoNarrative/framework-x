...
const ActiveItemsRemaining = component('ActiveItemsRemaining', createSub({ nActive: getNotDoneCount }), ({ nActive }) => <span className="todo-count">
        <strong>{nActive || 'No'}</strong> {`item${nActive === 1 ? '' : 's'}`} left
      </span>);
const ClearCompleted = component('ClearCompleted', { injectDispatch: true }, ({ dispatch }) => <button className="clear-completed" onClick={() => dispatch(evt.CLEAR_ALL_DONE)}>
      Clear completed
    </button>);
const Footer = component('Footer', createSub({ canClearCompleted: R.pipe(getDoneCount, n => n > 0), }), ({ canClearCompleted }) => (<footer className="footer">
      <ActiveItemsRemaining />
      <VisibilityFilters />
      {canClearCompleted && <ClearCompleted />}
    </footer>));
const MarkAllDone = component('MarkAllDone', createSub({
    nDone: getDoneCount,
    nTotal: getAllTodosCount,
}), ({ dispatch, nDone, nTotal }) => <span>
  <input className="toggle-all" type="checkbox" checked={nDone === nTotal} readOnly/>
...
}), ({ moreThanOneTodo }) => <section className={'main'}>
